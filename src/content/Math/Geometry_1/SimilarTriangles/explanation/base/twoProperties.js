...
export default class CommonCollectionTwoProp extends CommonDiagramCollection {
  ...
  side() {
    this._tri._pad2.scenarios.next = { position: [-2, -1] };
    this._tri._pad1.scenarios.next = { position: [2, -1] };
    let x = rand(-2.5, 2.5);
    const y = rand(0, 1);
    if (this.lastAuto === 'side') {
      if (Math.abs(x - this.lastValue) < 1) {
        x += Math.abs(x - this.lastValue) / (x - this.lastValue);
      }
    }
    this.lastValue = x;
    this.lastAuto = 'side';
  ...
  angle() {
    ...
    let x = rand(0, 2);
    if (this.lastAuto === 'angle') {
      if (Math.abs(x - this.lastValue) < 0.5) {
        if (this.lastValue < 1.25) {
          x = rand(1.5, 2.5);
        } else {
          x = rand(0, 1);
        }
    ...
    this.lastValue = x;
    this.lastAuto = 'angle';
    const angleToUse = Math.PI / 6;

    this._tri._pad0.scenarios.next = {
      position: [
        mag * Math.cos(angleToUse) + -2,
        mag * Math.sin(angleToUse) + -1,
      ],
    };
