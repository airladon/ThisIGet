...
export default class QuizCollection extends CommonQuizMixin(CommonDiagramCollection) {
  ...
  randomTriangle(
    ...
    // const sideRoot2 = side * Math.sqrt(2);
    // const height = 1 - 1 / Math.sqrt(2);
  ...
  showAnglesAndSides() {
    ...
    const answers = (value, index) => {
      const calcs = [
        ...
        round(value * Math.sqrt(2), precision),
        round(Math.sqrt(value), precision),
    ...
    if (unknownSide === '12') {
      ...
      if (knownSides.indexOf('01') > -1) {
        tri._side01.showAll();
        [correctAnswer, incorrect] = answers(side01, 2);
        incorrectAnswers = [...incorrectAnswers, ...incorrect];
      }
      if (knownSides.indexOf('20') > -1) {
        tri._side20.showAll();
        [correctAnswer, incorrect] = answers(side20, 6);
        incorrectAnswers = [...incorrectAnswers, ...incorrect];
      }
    }
    if (unknownSide === '01') {
      ...
      if (knownSides.indexOf('12') > -1) {
        tri._side12.showAll();
        [correctAnswer, incorrect] = answers(side12, 3);
        incorrectAnswers = [...incorrectAnswers, ...incorrect];
      }
      if (knownSides.indexOf('20') > -1) {
        tri._side20.showAll();
        [correctAnswer, incorrect] = answers(side20, 3);
        incorrectAnswers = [...incorrectAnswers, ...incorrect];
      }
    }
    if (unknownSide === '20') {
      ...
      if (knownSides.indexOf('12') > -1) {
        tri._side12.showAll();
        [correctAnswer, incorrect] = answers(side12, 6);
        incorrectAnswers = [...incorrectAnswers, ...incorrect];
      }
      if (knownSides.indexOf('01') > -1) {
        tri._side01.showAll();
