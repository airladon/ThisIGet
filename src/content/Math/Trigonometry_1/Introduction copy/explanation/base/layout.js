...
export default function diagramLayout() {
  ...
  const makeLine = (name, p1, p2, color, width = 0.03, arrow1 = false, arrow2 = false) => ({
    ...
    options: {
      ...
      color,
      width,
      arrowStart: arrow1 ? { width: 0.06, height: 0.08 } : null,
      arrowEnd: arrow2 ? { width: 0.06, height: 0.08 } : null,
    },
  });
  const line = {
    name: 'line',
    ...
    addElements: [
      ...
      makeLine(
        ...
        colors.components, 0.015, true, true,
      ),
      makeLine(
  ...
  const arrow = {
    ...
    method: 'collection',
    addElements: [
      makeLine('line', points[0], points[1], colors.line, 0.03, true, false),
      makeLine(
        'h', points[0].add(0.04, 0), points[2],
        colors.components, 0.015, true, false,
      ),
      makeLine(
        'v', points[2], points[1].add(0, -0.015),
        colors.components, 0.015, true, false,
  ...
  const cart = {
    ...
    mods: {
      scenarios: {
        cart: { position: points[1].add(0.5), scale: 1.2 },
      },
    },
  };
