...
deleted_accounts = \
    Users.query.filter(Users.username_hash == 'deleted account').all()

for user in users:
    if user.username_hash != 'deleted account':
        print('{:<10}{:<30}{:<40}{:<40}{:<5}'.format(
            user.id, user.get_username(), user.get_email(),
            str(user.signed_up_on), str(user.confirmed)))
